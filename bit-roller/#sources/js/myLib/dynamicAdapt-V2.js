...
function teleport(beforeFun = () => {}, afterFun = () => {}) {
  ...
  /*************************************************************************************
    ...
    где:
      "destination" - уникальный селектор куда перемещать либо home если нужно вернуть в текущую позицию;
    ...
    пример:
      data-tp="home > 990,1 | 989 > .header__container > 0,first"
...
//        console.log('медиа = ', itemRules.media, ' перемещаем - ', document.querySelector(itemRules.element), ` before №${itemRules.order} в `, document.querySelector(itemRules.destination));
  ...
  rules.forEach((itemRules) => {
    bpSet.add(itemRules ? itemRules.media : undefined);
  });
  ...
  bp.forEach((media) => {
    ...
    matchMedia.addEventListener ? matchMedia.addEventListener('change', (event) => {
      if (event.matches) {
        beforeFun(event);
