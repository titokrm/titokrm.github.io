...
export function handleMenuSlide({
  ...
  mobWidthSetting = 980,
  addHeaderPadding = true, // нужно добавлять padding-right хедеру для компенсации полосы прокрутки
  // при postion: fixed?
  ...
  scrollLockOldIoS = false, // блокировать скролл при открытом меню на старых iOS устройствах
} = {}) {
  // Сохраняем параметр в контракте функции для обратной совместимости, даже если он не используется в текущей версии.
  void mobWidthSetting;
  ...
  function setScrollLock(headerSelector = null, addHeaderPadding = false) {
    let heightScreen = document.documentElement.clientHeight;
    let heightBodyInner =
      document.querySelector(".wrapper-global").offsetHeight;
    if (
      ...
      scrollLocked = true;
      oldBodyInlineStyle = body.style.cssText;
      let bodyScrollTop = 0;
      if (scrollLockOldIoS && isTouchDevice()) {
        bodyScrollTop =
        ...
        body.style.cssText += `position: fixed; left: 0; width: 100%; touch-action: none; top: -${bodyScrollTop}px;`;
      ...
      oldStyleHeader = document.querySelector(headerSelector)
        ...
        : undefined;
      if (headerSelector !== null && addHeaderPadding && !isTouchDevice()) {
  ...
  function setScrollUnlock(headerSelector = null, addHeaderPadding = false) {
    // debugger;
    if (body.classList.contains("scroll-lock")) {
      body.classList.remove("scroll-lock");
      body.style.cssText = oldBodyInlineStyle;
      if (scrollLockOldIoS && isTouchDevice()) {
  ...
  /** *************************************************************************** */
  function handleHeaderScrollBehavior(
    ...
    .addEventListener("transitionend", (event) => {
      ...
      if (event.target.closest("." + menu)) {
        if (
          ...
          setScrollUnlock(headerSelector, addHeaderPadding);
  ...
  document.addEventListener("click", (event) => {
    if (
      ...
      setScrollLock(headerSelector, addHeaderPadding);
      if (addClassHeaderActive) {
        document
          .querySelector(headerSelector)
  ...
  if (changePosition || changeVisible) {
    handleHeaderScrollBehavior(headerSelector, changePosition, changeVisible);
  }
  if (closeOnResize) {
...
// Оставляем историческое имя экспорта, чтобы внешний код не ломался после рефакторинга.
export const menuSlide = handleMenuSlide;
