...
function menuSlide({
    ...
    if (event.target.closest('.' + menu)) {
      if (event.target.classList.contains(menu) && !event.target.classList.contains('active')) {
        ...
        if (addClassHeaderActive) {
          document.querySelector(headerSelector).classList.remove(addClassHeaderActive);
        }
        setTimeout(() => {
  ...
  document.addEventListener('click', (event) => {
    if (!document.querySelector('.' + menu).classList.contains('active') && (event.target.closest('.' + burger) || event.target.closest('.' + toggleMenu))) {
      ...
      if (addClassHeaderActive) {
        document.querySelector(headerSelector).classList.add(addClassHeaderActive);
      }
    }
  });
  document.addEventListener('click', (event) => {
  ...
  document.addEventListener('keydown', event => {
    if (event.code === 'Escape') {
      if (document.querySelector('.' + menu).classList.contains('active')) {
        ...
        togleBtn ? togleBtn.classList.remove('active') : null;
      }
    }
  });
